...
router.post('/', requireCustomerAuth, async (req: CustomerAuthRequest, res, next) => {
  try {
    ...
    const profile = await prisma.userProfile.findUnique({ where: { userId: req.customer!.id } });
    if (!profile?.countryCode || profile.countryCode.toUpperCase() === value.countryCode) return res.status(400).json({ success: false, error: { code: 'INTERNATIONAL_DESTINATION_REQUIRED', message: 'International travel requires a destination outside your profile country' } });
    const trip = await prisma.trip.create({ data: { ownerId: req.customer!.id, title: value.title, destination: value.destination, countryCode: value.countryCode, travelType: 'INTERNATIONAL', startAt: new Date(value.startAt), endAt: new Date(value.endAt), budgetMinor: value.budgetMinor ?? undefined, currency: value.currency, travelStyle: value.travelStyle || undefined, interests: value.interests, languages: value.languages, notes: value.notes || undefined, members: { create: { userId: req.customer!.id, role: 'OWNER' } } }, include: { members: true } });
...
router.get('/', requireCustomerAuth, async (req: CustomerAuthRequest, res, next) => {
  try {
    const trips = await prisma.trip.findMany({ where: { travelType: 'INTERNATIONAL', members: { some: { userId: req.customer!.id } } }, include: { members: { include: { user: { include: { profile: true } } } } }, orderBy: { startAt: 'asc' } });
