...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = siteSettings?.metaTitle || 'GoWith — Find someone to go with.';
  const description = siteSettings?.metaDescription || 'Discover trusted locals, travel companions and local experiences for your next journey.';
  return {
    title,
    description,
    openGraph: {
      ...
      siteName: 'GoWith',
      images: [{ url: siteSettings?.siteLogo ? `https://meetvia.com${siteSettings.siteLogo}` : 'https://meetvia.com/og-image.png', width: 1200, height: 630, alt: 'GoWith — Find someone to go with.' }],
    },
  };
}
...
export default async function Home() {
  ...
  const jsonLd = {
    ...
    name: 'GoWith',
    url: 'https://meetvia.com',
    description: 'Travel companions, trusted locals and local experiences.',
    logo: siteSettings?.siteLogo ? `https://meetvia.com${siteSettings.siteLogo}` : undefined,
  };
