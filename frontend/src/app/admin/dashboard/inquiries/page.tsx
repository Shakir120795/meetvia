  ...
  const handleStatusUpdate = async (newStatus: string) => {
    ...
    try {
      ...
      const res = await adminPut<ApiResponse<IContactInquiry>>(
        `/api/v1/admin/inquiries/${selectedInquiry._id}`,
        ...
        <DataTable
          ...
          actions={(item) => (
            <button
              ...
              aria-label={`View inquiry from ${item.fullName}`}
            >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <p className="text-sm text-white">{selectedInquiry.fullName}</p>
              </div>
              <div>
                <p className="text-xs text-white/50 mb-0.5">Email</p>
                <p className="text-sm text-white">{selectedInquiry.email}</p>
              </div>
              <div>
                <p className="text-xs text-white/50 mb-0.5">Mobile</p>
                <p className="text-sm text-white">{selectedInquiry.mobile || '—'}</p>
              </div>
              <div>
                <p className="text-xs text-white/50 mb-0.5">Service Type</p>
                <p className="text-sm text-white">{selectedInquiry.serviceType}</p>
              </div>
              <div>
                <p className="text-xs text-white/50 mb-0.5">Preferred Date</p>
              ...
              <div>
                ...
                <p className="text-sm text-white">{formatDate(selectedInquiry.createdAt)}</p>
              </div>
            </div>

            {/* Message */}
            <div>
              <p className="text-xs text-white/50 mb-0.5">Message</p>
            ...
            {/* Safety confirmed */}
            <div>
              <p className="text-xs text-white/50 mb-0.5">Safety Confirmed</p>
              <span
                className={`text-xs px-2 py-0.5 rounded-full border ${
                  selectedInquiry.safetyConfirmed
                    ? 'bg-green-500/20 text-green-400 border-green-500/30'
                    : 'bg-red-500/20 text-red-400 border-red-500/30'
                }`}
              >
                {selectedInquiry.safetyConfirmed ? 'Yes' : 'No'}
              </span>
            </div>
            ...
            <div className="flex gap-2 pt-2 border-t border-white/10">
              {selectedInquiry.mobile && (
                <a
                  href={`https://wa.me/${selectedInquiry.mobile.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(`Hi ${selectedInquiry.fullName}, this is Meetvia team regarding your ${selectedInquiry.serviceType} inquiry.`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 px-3 py-2 bg-green-600 hover:bg-green-700 text-white text-xs font-medium rounded-theme transition-colors"
                >
                  💬 WhatsApp
                </a>
              )}
              <a
                href={`mailto:${selectedInquiry.email}?subject=Regarding your Meetvia inquiry&body=Hi ${selectedInquiry.fullName},%0A%0AThank you for your interest in our ${selectedInquiry.serviceType} service.`}
                className="flex items-center gap-1.5 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-theme transition-colors"
              >
                ✉️ Email
              </a>
            </div>
