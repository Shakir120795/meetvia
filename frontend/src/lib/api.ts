...
export async function publicGet<T>(path: string): Promise<T> { return handleResponse<T>(await fetch(path, { method: 'GET', headers: { 'Content-Type': 'application/json' } })); }
export async function publicPost<T>(path: string, body: unknown): Promise<T> { return handleResponse<T>(await fetch(path, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })); }

function getAuthHeaders(): HeadersInit { const token = getToken(); const headers: HeadersInit = { 'Content-Type': 'application/json' }; if (token) headers['Authorization'] = `Bearer ${token}`; return headers; }
...
function getCustomerHeaders(): HeadersInit { const session = getCustomerSession(); const headers: HeadersInit = { 'Content-Type': 'application/json' }; if (session) headers['Authorization'] = `Bearer ${session.token}`; return headers; }
...
export async function customerPost<T>(path: string, body?: unknown): Promise<T> { return handleResponse<T>(await fetch(path, { method: 'POST', headers: getCustomerHeaders(), body: body === undefined ? undefined : JSON.stringify(body) })); }
export async function customerPatch<T>(path: string, body: unknown): Promise<T> { return handleResponse<T>(await fetch(path, { method: 'PATCH', headers: getCustomerHeaders(), body: JSON.stringify(body) })); }
