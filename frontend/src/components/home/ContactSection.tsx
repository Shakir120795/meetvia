  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await submitContact({
        ...
        mobile: String(form.get('mobile') || '').trim() || undefined,
      ...
      setError(submissionError instanceof Error ? submissionError.message : 'Unable to submit your inquiry. Please try again.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <section ref={sectionRef} className="py-20 px-4 sm:px-6 lg:px-8" id="contact">
      <div className="max-w-5xl mx-auto">
        ...
        >
          <h2 className="text-3xl sm:text-4xl font-bold text-primary mb-4">Contact GoWith</h2>
          <p className="text-primary/70 text-lg max-w-2xl mx-auto">
            Have a question about finding a companion or joining GoWith? Send us your details and we&apos;ll get back to you.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_0.8fr] gap-8">
          <motion.form
            ...
            className="glass p-6 sm:p-8 rounded-theme space-y-5"
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label className="text-sm font-medium text-primary">
                Full name
                <input name="fullName" required maxLength={100} className={`${inputClass} mt-2`} />
              </label>
              <label className="text-sm font-medium text-primary">
                Email
                <input name="email" type="email" required className={`${inputClass} mt-2`} />
              </label>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label className="text-sm font-medium text-primary">
                Mobile
                <input name="mobile" type="tel" className={`${inputClass} mt-2`} />
              </label>
              <label className="text-sm font-medium text-primary">
                What do you need?
                <select name="serviceType" required className={`${inputClass} mt-2`} defaultValue="">
                  <option value="" disabled>Select an option</option>
                  <option value="companion">Find a companion</option>
                  <option value="experience">Experience</option>
                  <option value="become-companion">Become a companion</option>
            ...
            <label className="text-sm font-medium text-primary block">
              Preferred date <span className="font-normal text-primary/50">(optional)</span>
              <input name="preferredDate" type="date" className={`${inputClass} mt-2`} />
            </label>

            <label className="text-sm font-medium text-primary block">
              Message
              <textarea name="message" required maxLength={2000} rows={5} className={`${inputClass} mt-2 resize-y`} />
            </label>

            <label className="flex items-start gap-3 text-sm text-primary/70">
              <input type="checkbox" required className="mt-1 h-4 w-4" />
              <span>I agree that GoWith may use these details to respond to my inquiry.</span>
            </label>

            {error && <p role="alert" className="text-sm text-red-600">{error}</p>}
            {submitted && (
              <p role="status" className="flex items-center gap-2 text-sm text-green-700">
                <CheckCircle2 className="h-4 w-4" /> Your inquiry has been submitted successfully.
              </p>
            )}
          ...
          >
            ...
            <p className="text-primary/70 text-sm mb-6">
              Prefer a direct conversation? Message us on WhatsApp.
            </p>
            <Button href={whatsappLink} variant="primary" size="md" className="bg-green-600 hover:bg-green-700">
              Message on WhatsApp
            </Button>
          </motion.div>
        </div>
