...
export default function ContactForm({ siteSettings, services }: ContactFormProps) {
  ...
  const onSubmit = async (data: ContactFormData) => {
    ...
    try {
      const response = await fetch('/api/v1/public/contact', {
        ...
        body: JSON.stringify({ ...data, safetyConfirmed: true }),
      });
    ...
    return (
      <div className="glass p-8 text-center space-y-6">
        ...
        {siteSettings.whatsappNumber && (
          <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white font-medium px-6 py-3 rounded-theme transition-colors duration-200">
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="glass p-6 md:p-8 space-y-5" noValidate>
      ...
      <div className="flex flex-col gap-1.5">
        <label htmlFor="preferredDate" className="text-sm font-medium text-foreground/80">Preferred Date</label>
        <input id="preferredDate" type="date" min={new Date().toISOString().slice(0, 10)} className="w-full bg-white/5 border border-white/10 rounded-theme px-4 py-2.5 text-foreground focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50 transition-colors duration-200" {...register('preferredDate')} />
      </div>
      ...
      <div className="space-y-1.5">
        <label className="flex items-start gap-3 cursor-pointer">
          <input type="checkbox" checked={safetyChecked} onChange={(e) => { setSafetyChecked(e.target.checked); if (e.target.checked) setSafetyError(''); }} className="mt-1 w-4 h-4 rounded border-white/20 bg-white/5 text-accent focus:ring-accent/50 flex-shrink-0" />
          <span className="text-sm text-foreground/70">{siteSettings.safetyCheckboxText || 'I confirm that I understand MeetVia provides public-only companionship and city assistance services. All meetings occur in public places.'}</span>
        </label>
        {safetyError && <p className="text-sm text-red-500 ml-7" role="alert">{safetyError}</p>}
      </div>
