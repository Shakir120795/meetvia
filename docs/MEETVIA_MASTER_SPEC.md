...
## Release order
Foundation → Authentication → Companion Marketplace → Experience Marketplace → Verification → Booking → Payment → Chat → Safety/Moderation → Admin Control → Notifications → Reviews → QA → Production → Growth/Future Features.
...
## MVP definition
Landing + Authentication + Companion Marketplace + Experience Marketplace + Verification + Booking + Payment + Booking-controlled Chat + Basic Safety + Admin Dashboard + Notifications + Reviews + Legal/Safety.
...
Use feature flags for advanced modules, e.g. `ENABLE_AI_MATCHING=false`, `ENABLE_REFERRALS=false`, `ENABLE_SUBSCRIPTIONS=false`, `ENABLE_DYNAMIC_PRICING=false`.
